...
#x_val =st.sidebar.selectbox("Pick your x-axis", list(who_data.columns))
#y_val =st.sidebar.selectbox("Pick your y-axis", list(who_data.columns))
...
scatter = alt.Chart(who_data, title= f"Correlation between {x_val} and {y_val}").mark_point().encode(
    alt.X(x_val, title= "{x_val}}"),
    alt.Y(y_val, title= "{y_val}"), 
    tooltip= [x_val,y_val]
)
